...
class rbs_documento_propiedad(models.Model):
    ...
    state_filedata = fields.Selection([
            ...
            ('done', 'Listo'),
            ('error', 'Error'),
        ], default='empty', string='Estado del archivo', readonly=True, states={'draft': [('readonly', False)]})
    ...
    def procesar_pdf(self):
        for propiedad in self:
            ...
            try:
                ...
                contenedor = self.env['rbs.contenedor'].create(
                    {'name': str(propiedad.anio_id.name) +
                        str(propiedad.libro_id.name) +
                        str(propiedad.tomo_id.name) +
                        str(propiedad.numero_inscripcion) +
                        str("propiedad")})
    ...
    def procesar_pdf(self):
        for mercantil in self:
            ...
            try:
                ...
                contenedor = self.env['rbs.contenedor'].create(
                    {'name': str(mercantil.anio_id.name) +
                        str(mercantil.libro_id.name) +
                        str(mercantil.tomo_id.name) +
                        str(mercantil.numero_inscripcion) +
                        str("mercantil")})
